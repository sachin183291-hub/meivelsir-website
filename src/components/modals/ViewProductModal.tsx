...
import { X, ExternalLink, ChevronLeft, ChevronRight, Play, Pause, Package } from "lucide-react";
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      <motion.div 
        ...
        className="relative w-full max-w-5xl max-h-[90vh] bg-card rounded-2xl shadow-2xl overflow-hidden border border-border/50 flex flex-col md:flex-row z-10"
      >
        {/* Close Button */}
        <button 
          onClick={onClose} 
        ...
        <div className={`w-full ${hasImages ? 'md:w-1/2' : ''} p-8 md:p-10 overflow-y-auto max-h-[90vh]`}>
          ...
          <div className="flex flex-wrap gap-2 mb-8">
            <span className="px-3 py-1 bg-accent text-accent-foreground text-sm font-bold rounded-full shadow-sm">
              {product.status}
            </span>
            <span className="px-3 py-1 bg-primary/20 text-primary text-sm font-bold rounded-full shadow-sm">
              {product.year}
            </span>
          </div>
          ...
          <div className="space-y-6">
            <div className="pt-2">
              <h3 className="text-sm font-semibold text-foreground/80 uppercase tracking-wider mb-4 flex items-center gap-2">
                <Package className="w-4 h-4" /> About the Product
              </h3>
            ...
            {product.tech && product.tech.length > 0 && (
              <div className="pt-6 border-t border-border/50">
                <h3 className="text-sm font-semibold text-foreground/80 uppercase tracking-wider mb-4">Technologies</h3>
                <div className="flex flex-wrap gap-2">
                  {product.tech.map(tech => (
                    <span key={tech} className="text-sm px-3 py-1.5 bg-muted text-muted-foreground rounded-lg font-medium border border-border">
                      {tech}
                    </span>
                  ))}
                </div>
